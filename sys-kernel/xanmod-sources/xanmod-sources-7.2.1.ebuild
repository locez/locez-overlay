...
SRC_URI="
	...
	${GENPATCHES_URI}
	${ARCH_URI}
	https://downloads.sourceforge.net/project/xanmod/releases/edge/${PV}-xanmod${XANMOD_VERSION}/patch-${PV}-xanmod${XANMOD_VERSION}.xz
"
...
RDEPEND="
	zfs? (
		|| (
			( >=sys-fs/zfs-kmod-2.3.9 <sys-fs/zfs-kmod-2.4.0_rc1 )
			>=sys-fs/zfs-2.4.4[modules]
		)
	)
"
...
src_prepare() {
	...
	if [[ ${KV_MAJOR}.${KV_MINOR} == 7.2 ]]; then
		eapply "${FILESDIR}/${PN}-7.2-x86-native-cache-shift.patch"
	else
